 ...
 */
@Slf4j
public class Db2StandaloneSqlSplitterxxx {
    ...
    public Db2StandaloneSqlSplitterxxx() {
        this(DEFAULT_SQL_DELIMITER);
    }

    public Db2StandaloneSqlSplitterxxx(String delimiter) {
        this(delimiter, true);
    }

    public Db2StandaloneSqlSplitterxxx(String delimiter, boolean addDelimiter) {
...
//                    continue;
//                }
//                if (Objects.equals(text, delimiter)) {
...
//            }
    ...
    private int addStmtWhileStmtEnd(Token[] tokens, int pos) {
        ...
        if (StringUtils.isNotBlank(currentStmt)) {
            ...
            // DB2 behavior: custom terminator applies to the just-ended statement only in typical scripts.
            // After consuming the non-default terminator token, reset delimiter back to default ';'
            // so that the following SQLs use the normal terminator again.
            if (!DEFAULT_SQL_DELIMITER.equals(this.delimiter)) {
                this.delimiterTokens = innerUtils.extractDelimiterTokens(DEFAULT_SQL_DELIMITER);
                this.delimiter = DEFAULT_SQL_DELIMITER;
            }
            if (notDefaultSqlDelimiter) {
    ...
    private static class Db2SqlSplitterIterator implements SqlStatementIterator {
        ...
        private OffsetString parseNext() {
            try {
                ...
                while (this.holder.isEmpty() && (line = this.reader.readLine()) != null) {
                    ...
                    Db2StandaloneSqlSplitterxxx splitter = createSplitter();
        ...
        private Db2StandaloneSqlSplitterxxx createSplitter() {
            return new Db2StandaloneSqlSplitterxxx(this.delimiter, addDelimiter);
        }
    }
}
