...
public class SqlSplitterIteratorTest {
    ...
    protected void verifyByFileName(String fileName) {
        ...
        SqlSplitter sqlSplitter = sqlSplitter();
        List<String> except =
                sqlSplitter.split(testData.getOrigin()).stream().map(OffsetString::getStr).collect(Collectors.toList());
        SqlStatementIterator sqlStatementIterator = sqlStatementIterator(testData.getOrigin());
        List<String> actual = new ArrayList<>();
        while (sqlStatementIterator.hasNext()) {
            actual.add(sqlStatementIterator.next().getStr());
        }
    ...
    private List<OffsetString> getSqls(String fileName) {
        // 需要手动解析YAML以正确映射到OffsetString对象
        try {
            String filePath = fileName.replace("src/test/resources/", "");
            InputStream inputStream = getClass().getClassLoader().getResourceAsStream(filePath);
            org.yaml.snakeyaml.Yaml yaml = new org.yaml.snakeyaml.Yaml(new org.yaml.snakeyaml.constructor.Constructor(List.class));
            @SuppressWarnings("unchecked")
            List<Object> rawList = yaml.load(inputStream);
            List<OffsetString> results = new ArrayList<>();
            ...
            for (Object item : rawList) {
                if (item instanceof Map) {
                    ...
                    results.add(new OffsetString(offset, str));
                }
            }
            return results;
        } catch (Exception e) {
